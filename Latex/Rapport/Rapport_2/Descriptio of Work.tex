...
\begin{itemize}
  \item Ressource consumption (RAM, GPU, CPU, time)
        \subitem Size of \href{https://www.youtube.com/watch?v=dQw4w9WgXcQ}{this youtube video} in 360p: 8.68 MB
        \subitem Size of the same video as an mp3
\end{itemize}
...
{\color{blue}
  SNNs, when well constructed, offer the following benefits
  ...
  - Robustness to noise: SNNs are inherently robust to noise and can effectively filter out irrelevant information. This makes them useful in environments where noise is a common problem, in our case when the audio is not well recorded.
  ...
  - SNNs are difficult to train, mainly due to backpropagation issues
...
- SNNs might not be the best method to process audio data.
